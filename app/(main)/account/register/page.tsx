...
export default function Register() {
  ...
  async function submit(event: FormEvent) {
    ...
    if (!response.ok) {
      ...
      setError(data.error || 'Account creation could not be completed. Please try again.')
      return
    }
  ...
  if (confirmationSent) return <main className="flex min-h-screen items-center justify-center bg-[#f8f7f3] px-4 py-16"><section className="w-full max-w-md rounded-xl border border-[#e6e4df] bg-white p-8 text-center shadow-sm"><CheckCircle2 className="mx-auto h-12 w-12 text-emerald-600"/><h1 className="mt-5 font-display text-3xl font-bold text-ink">Check your email</h1><p className="mt-3 text-sm leading-6 text-slate-600">We sent a confirmation link to <strong>{form.email}</strong>. Open it to activate your account, then sign in.</p><Link href="/account/login" className="mt-6 inline-flex rounded-lg bg-primary px-5 py-3 text-sm font-bold text-white">Go to sign in</Link></section></main>

  return <main className="min-h-screen bg-[#f8f7f3] px-4 py-16"><section className="mx-auto max-w-md rounded-xl border border-[#e6e4df] bg-white p-6 shadow-sm sm:p-8">
    ...
    <div className="mt-7"><GoogleSignInButton next="/"/></div><div className="my-5 flex items-center gap-3 text-xs font-semibold uppercase tracking-wider text-slate-400"><span className="h-px flex-1 bg-[#e6e4df]"/><span>or use email</span><span className="h-px flex-1 bg-[#e6e4df]"/></div>
