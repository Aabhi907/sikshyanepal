...
export default async function AdmissionStatusPage() {
  const { data } = await createServerSupabaseClient().from('admissions').select('*, school:schools(id,name,slug,district,province), college:colleges(id,name,slug,location)').eq('status', 'published').order('application_open_at', { ascending: true, nullsFirst: false }).order('application_deadline', { ascending: true, nullsFirst: false }).limit(300)
