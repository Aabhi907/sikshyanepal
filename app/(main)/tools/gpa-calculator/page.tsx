...
export default function GpaCalculatorPage() {
  ...
  return <div className="min-h-screen bg-[#f0f4ff]">
    ...
    <main className="mx-auto grid max-w-4xl gap-6 px-4 py-8 sm:px-6 lg:grid-cols-[1fr_280px]">
      <section className="rounded-2xl border border-gray-200 bg-white p-5 sm:p-7"><div className="flex items-center justify-between gap-3"><h2 className="font-display text-xl font-bold text-ink">Your subjects</h2><button onClick={() => setSubjects(initial())} className="inline-flex items-center gap-1.5 text-sm font-semibold text-gray-500 hover:text-primary"><RotateCcw className="h-4 w-4" />Reset</button></div><div className="mt-5 space-y-3">{subjects.map((subject, index) => <div key={subject.id} className="grid gap-2 rounded-xl bg-gray-50 p-3 sm:grid-cols-[1fr_130px_80px_32px] sm:items-center"><input value={subject.name} onChange={event => update(subject.id, { name: event.target.value })} placeholder={`Subject ${index + 1}`} className="rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-ink outline-none focus:border-primary" /><select value={subject.grade} onChange={event => update(subject.id, { grade: event.target.value })} className="rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-ink outline-none focus:border-primary"><option value="">Select grade</option>{grades.map(grade => <option key={grade.label} value={grade.label}>{grade.label} — {grade.point.toFixed(1)}</option>)}</select><input type="number" min="0" step="0.5" aria-label={`Credit for ${subject.name || `subject ${index + 1}`}`} value={subject.credit} onChange={event => update(subject.id, { credit: Number(event.target.value) })} className="rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-ink outline-none focus:border-primary" />{subjects.length > 1 ? <button onClick={() => setSubjects(all => all.filter(item => item.id !== subject.id))} aria-label={`Remove ${subject.name || 'subject'}`} className="rounded p-2 text-gray-400 hover:bg-red-50 hover:text-red-600"><Trash2 className="h-4 w-4" /></button> : <span />}</div>)}</div><button onClick={add} className="mt-5 inline-flex items-center gap-2 rounded-xl border border-primary px-4 py-2.5 text-sm font-bold text-primary hover:bg-blue-50"><Plus className="h-4 w-4" />Add subject</button><p className="mt-5 text-xs leading-5 text-gray-500">Use credit 1 for equal-weight subjects. For a course with theory and practical components, add them separately or use the official credit weighting.</p></section>
