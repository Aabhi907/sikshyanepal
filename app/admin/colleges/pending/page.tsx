...
export default function PendingCollegesPage() {
  ...
  function confirmApprove(id: string, name: string) {
    setDialog({
      isOpen: true,
      title: 'Approve College',
      message: `Approve "${name}"? It will become publicly visible on the colleges listing immediately.`,
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          if (!res.ok) {
            ...
            toast.error(err.error || 'Failed to approve college')
            return
          }
          setColleges((c) => c.filter((x) => x.id !== id))
          toast.success(`"${name}" approved — now live on the site`)
        } catch {
          toast.error('Network error — college not approved')
        } finally {
          setWorking(null)
        }
      },
  ...
  function confirmReject(id: string, name: string) {
    setDialog({
      isOpen: true,
      title: 'Reject & Delete',
      message: `Reject and permanently delete "${name}"? This cannot be undone.`,
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          if (!res.ok) {
            const err = await res.json()
            toast.error(err.error || 'Failed to reject college')
            return
          }
          setColleges((c) => c.filter((x) => x.id !== id))
          toast.success(`"${name}" rejected and deleted`)
        } catch {
          toast.error('Network error — college not rejected')
        } finally {
          setWorking(null)
        }
      },
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  {c.submitted_by && (
                    <div className="flex items-center gap-2 text-xs text-gray-500 border-t border-gray-700/60 pt-3">
                      ...
                      <span>
                        Submitted by{' '}
                        <span className="text-gray-300 font-semibold">{c.submitted_by}</span>
                ...
                <div className="flex flex-col gap-2 flex-shrink-0">
                  ...
                  <button
                    onClick={() => confirmReject(c.id, c.name)}
                    disabled={working === c.id}
                    className="flex items-center gap-1.5 px-4 py-2 bg-gray-700 hover:bg-red-900/60 text-gray-300 hover:text-red-300 text-sm font-semibold rounded-lg transition-colors disabled:opacity-50"
                  >
