...
type Item = { id: string; title: string; target_type: string; scraper_name: string; source_url: string; fetched_at: string; quality_flags: string[]; status: string; payload: Record<string, unknown>; confidence_score?: number; content_hash?: string | null; raw_snapshot?: string | null; reviewer_notes?: string | null }
...
export default function IngestionPage() {
  ...
  return <div className="p-8 text-gray-100">
    ...
    <div className="mt-6 grid gap-4 xl:grid-cols-2">{items.map(item => <article key={item.id} className="rounded-xl border border-gray-700 bg-gray-800 p-5">
      <div className="flex items-start justify-between gap-3"><div><div className="flex flex-wrap gap-2"><span className="rounded bg-gray-700 px-2 py-1 text-[10px] font-bold uppercase text-cyan-300">{item.target_type}</span><span className={`rounded px-2 py-1 text-[10px] font-bold uppercase ${Number(item.confidence_score || 0) >= 90 ? 'bg-emerald-950/60 text-emerald-300' : Number(item.confidence_score || 0) >= 70 ? 'bg-amber-950/60 text-amber-300' : 'bg-red-950/60 text-red-300'}`}>Confidence {item.confidence_score ?? 0}/100</span></div><h2 className="mt-2 font-semibold text-white">{item.title}</h2><p className="mt-1 text-xs text-gray-500">{item.scraper_name} · {new Date(item.fetched_at).toLocaleString()}</p></div><a href={item.source_url} target="_blank" rel="noopener noreferrer" className="rounded p-2 text-blue-400" title="Open original source"><ExternalLink className="h-4 w-4" /></a></div>
      {item.quality_flags?.length > 0 && <div className="mt-3 flex flex-wrap gap-2">{item.quality_flags.map(flag => <span key={flag} className={`rounded px-2 py-1 text-xs ${flag === 'source_changed' ? 'bg-blue-950/70 font-semibold text-blue-300' : 'bg-amber-950/60 text-amber-300'}`}>{flag.replaceAll('_', ' ')}</span>)}</div>}
      ...
      {item.raw_snapshot && <details className="mt-3 rounded-lg border border-gray-700 bg-gray-900/60 p-3"><summary className="cursor-pointer text-xs font-semibold text-gray-300">Source snapshot {item.content_hash ? `· ${item.content_hash.slice(0, 12)}…` : ''}</summary><p className="mt-3 max-h-52 overflow-auto whitespace-pre-wrap text-xs leading-5 text-gray-300">{item.raw_snapshot}</p><p className="mt-2 text-[11px] text-amber-300">For verification only—write an original summary rather than republishing the source text.</p></details>}
