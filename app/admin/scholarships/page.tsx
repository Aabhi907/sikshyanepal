    ...
    setDialog({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          if (!res.ok) { toast.error('Failed to delete scholarship'); return }
          setItems((prev) => prev.filter((s) => s.id !== id))
          toast.success('Scholarship deleted')
        } catch {
          toast.error('Network error — scholarship not deleted')
        } finally {
          setWorking(null)
        }
      },
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2">
            <Award className="w-6 h-6 text-green-400" /> Scholarships
          </h1>
              ...
              {items.length === 0 ? (
                <tr>
                  <td colSpan={4} className="text-center py-10 text-gray-500">
                    No scholarships.{' '}
                    <Link href="/admin/scholarships/new" className="text-blue-400 hover:underline">Add one</Link>
                  </td>
                </tr>
              ) : (
                items.map((item) => (
                  <tr key={item.id} className="border-b border-gray-700/50">
                    ...
                    <td className="px-5 py-3 text-gray-400 hidden md:table-cell">
                      {item.college?.name || 'General'}
                    </td>
                    <td className="px-5 py-3 text-green-400 font-medium hidden md:table-cell">
                      {item.amount ? `NPR ${item.amount.toLocaleString()}` : '—'}
                    </td>
                    <td className="px-5 py-3">
