...
export default function AdminStudyResourcesPage() {
  ...
  async function toggle(item: Resource) { await fetch(`/api/admin/study-resources/${item.kind}/${item.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ is_published: !item.is_published }) }); load() }
  async function remove(item: Resource) { if (!window.confirm('Delete this resource?')) return; await fetch(`/api/admin/study-resources/${item.kind}/${item.id}`, { method: 'DELETE' }); load() }
