...
export async function POST(request: Request) {
  ...
  const { data, error } = await supabase
    .from('reviews')
    .insert({ college_id, student_name, program, year: year || null, rating, review_text, is_approved: false, teaching_rating: body.teaching_rating || null, facilities_rating: body.facilities_rating || null, administration_rating: body.administration_rating || null, value_rating: body.value_rating || null, placement_rating: body.placement_rating || null, attendance_rating: body.attendance_rating || null, safety_rating: body.safety_rating || null, internship_support_rating: body.internship_support_rating || null, hidden_costs_reported: Boolean(body.hidden_costs_reported), hostel_transport_note: body.hostel_transport_note?.trim().slice(0, 300) || null, verification_status: evidence_url ? 'submitted' : 'unverified' })
    .select()
    .single()
