...
export default function CommunityMedia({ url, type, compact = false }: { url?: string | null; type?: 'image' | 'video' | null; compact?: boolean }) {
  ...
  return <div className={`relative w-full overflow-hidden rounded-xl bg-gray-100 ${compact ? 'h-56' : 'h-[22rem] sm:h-[30rem]'}`}><Image src={url} alt="Media shared with this anonymous community post" fill sizes={compact ? '(max-width: 1024px) 100vw, 620px' : '(max-width: 768px) 100vw, 768px'} className="object-contain" /></div>
}
