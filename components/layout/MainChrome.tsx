...
export default function MainChrome({ children }: { children: React.ReactNode }) {
  ...
  if (AUTH_PAGES.includes(pathname)) return <div className="min-h-screen bg-[#f8f7f3]">{children}</div>
  return <><Header/><SiteAnnouncement/><div className="min-h-screen bg-gray-50 pb-20 lg:pb-0">{children}</div><Footer/><MobileQuickNav/><GlobalSearch/></>
}
